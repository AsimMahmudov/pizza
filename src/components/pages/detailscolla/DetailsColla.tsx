...
import scss from "./DetailsColla.module.scss";
import axios from "axios";
import { AddBasket, AddProduct } from "@/redux/ProductSlice";

const pizza = process.env.NEXT_PUBLIC_COLLA;
...
const DetailsColla = () => {
  ...
  const handleBigClick = () => {
    if (originalPrice !== null) {
      const newPrice = originalPrice * 3; // Triple the price after resetting
  ...
  const handleSmallClick = () => {
    if (originalPrice !== null) {
      setCurrentPrice(originalPrice); // Reset to original price
    }
  };

  const handleAverageClick = () => {
    if (originalPrice !== null) {
      const newPrice = originalPrice * 2; // Double the price after resetting
      setCurrentPrice(newPrice);
    }
  };
            ...
            <div className={scss.boxtext}>
              ...
              <div className={scss.boxbtn}>
                <div className={scss.btn}>
                  ...
                  <button onClick={handleAverageClick} className={scss.average}>
                    average
                  </button>
                ...
                <button className={scss.btnBasket} onClick={handleAddToBasket}>
                  {currentPrice} сом
                </button>
              </div>
            </div>
